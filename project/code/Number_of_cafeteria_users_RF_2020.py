...
date_train_lunch_mask = date_train_lunch['날짜'] >= '2020-01-20'
date_train_dinner_mask = date_train_dinner['날짜'] >= '2020-01-20'

date_train_lunch = date_train_lunch.loc[date_train_lunch_mask, :]
date_train_dinner = date_train_dinner.loc[date_train_dinner_mask, :]

train_mask = train['일자'] >= '2020-01-20'

train = train.loc[train_mask, :]

# 1월 20일
corona_train = pd.read_csv('./save/data/preprocess/corona_train.csv')
...
# x1_train = pd.concat([x_train, lunch_train, corona_train], axis=1, ignore_index=True)
# x2_train = pd.concat([x_train, dinner_train, corona_train], axis=1, ignore_index=True)

lunch_train.reset_index(drop=True, inplace=True)
dinner_train.reset_index(drop=True, inplace=True)
...
# 코로나 발생 이후 2020년부터를 훈련데이터로 잡았을 경우
# r2 score :  0.8161440722896985